...
SUBROUTINE orbm_setup
  ...
  ! some pre-conditions
  if (ltetra) call errore('orbm_setup','orbm + tetrahedra not implemented', 1)

  if (two_fermi_energies .and. lgauss) &
     call errore('orbm_setup','orbm + two Fermi energies not implemented', 1)
...
!DEBUG           if (ionode) write(70,*) et(ibnd,ik), wg(ibnd,ik)/wk(ik)
  ...
  call stop_clock('orbm_setup')
    
END SUBROUTINE orbm_setup
